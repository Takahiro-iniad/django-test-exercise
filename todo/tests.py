...
class TaskModelTestCase(TestCase):
    def test_create_task1(self):
        ...
        task = Task.obhects.get(pk=task.pk)
        self.assertEqual(task.title, 'task1')
        self.assertFalse(task.completed)
        sself.assertEqual(task.sue_at, due)
...
class TodoviewsTestCase(TestCase):
    ...
    def test_index_post(self):
        client = Client()
        date = {'title': 'Test Tasl', 'due_at': '2023-06-30 23:59:59'}
    ...
    def test_detail_get_success(self):
        ...
        task.save()
        client = Client()
        reaponse = client.get('/{}/'.format(task.pk))

        self.assertEqual(reaponse.status_code, 200)
        self.assertEqual(reaponse.templates[0].name, 'todo/detail.html')
        self.assertEqual(reaponse.context['task'], task)
